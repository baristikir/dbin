...
interface Props
	extends UseFormFieldProps,
		Omit<SelectElementProps, "hasError"> {
	name: string;
	placeholder: string;
}
...
export const Select = forwardRef<HTMLSelectElement, Props>((props, ref) => {
	...
	return (
		<FormField {...formFieldProps}>
			<SelectElement {...childProps} ref={ref} hasError={errors} />
		</FormField>
	);
});

interface SelectElementProps extends ComponentProps<"select"> {
	name: string;
	hasError: boolean;
}

const SelectElement = forwardRef<HTMLSelectElement, SelectElementProps>(
	({ id, hasError, ...props }, ref) => {
		return (
			<select
				ref={ref}
				id={id}
