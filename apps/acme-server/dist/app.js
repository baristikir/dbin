...
var BusinessCredentialSchema = mod.object({
  registered_name: mod.string().min(1),
  registered_adress: mod.string().min(1),
  registered_country: mod.string().min(1),
  created_at: mod.date().max(new Date())
});
builder.mutationField(
  "proposeCredential",
  (t) => t.field({
  ...
  fields: (t) => ({
    ...
    connections: t.field({
      ...
      resolve: (_root, _args, { agent: agent2 }) => {
        return agent2.connections.getAll();
      }
    }),
    credentials: t.field({
